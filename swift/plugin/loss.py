...
def stream_focal_loss(outputs, labels, special_tokens=None, 
                                 gamma=2.0, auto_alpha=True, num_items_in_batch=None, 
                                 trainer=None, **kwargs):
    """
    Focal loss for streaming video analysis with special tokens.
    
    Args:
        outputs: Model outputs containing logits
        labels: Target labels
        special_tokens: List of special token IDs. If None, will be dynamically 
                       retrieved from tokenizer using </Silence>, </Standby>, </Response>
        gamma: Focal loss gamma parameter (default: 2.0)
        auto_alpha: Whether to automatically balance class weights (default: True)
        num_items_in_batch: Number of items for loss normalization
        trainer: Trainer instance to access tokenizer for dynamic token ID retrieval
    """
    logits = outputs.logits
    device = logits.device
    dtype = logits.dtype 
    
    # Dynamically get special token IDs from tokenizer if not provided
    if special_tokens is None:
        if trainer is not None and hasattr(trainer, 'processing_class'):
            tokenizer = trainer.processing_class
            special_tokens = []
            for token_str in ['</Silence>', '</Standby>', '</Response>']:
                try:
                    token_ids = tokenizer.encode(token_str, add_special_tokens=False)
                    if token_ids:
                        special_tokens.append(token_ids[0])
                except Exception as e:
                    print(f"Warning: Failed to encode {token_str}: {e}")
            if not special_tokens:
                print("Warning: No special tokens found, falling back to standard cross entropy")
                special_tokens = []
        else:
            print("Warning: trainer not provided, cannot dynamically get special token IDs")
            special_tokens = []
    ...
    shift_logits_masked = shift_logits[masks]
    shift_labels_masked = shift_labels[masks]
    
    is_special = torch.zeros_like(shift_labels_masked, dtype=torch.bool)
    for special_token in special_tokens:
        is_special |= (shift_labels_masked == special_token)
    
    loss_fct = CrossEntropyLoss(reduction='none')
    ce_loss = loss_fct(shift_logits_masked, shift_labels_masked)
    
    weights = torch.ones_like(ce_loss, dtype=torch.float32)
    
    if is_special.any():
        probs = torch.softmax(shift_logits_masked, dim=-1)
        arange_idx = torch.arange(shift_labels_masked.size(0), device=device)
        pt = probs[arange_idx, shift_labels_masked]
        
        focal_weight = torch.pow(1.0 - pt, gamma)
        
        if auto_alpha:
            special_targets = shift_labels_masked[is_special]
            unique_tokens, counts = torch.unique(special_targets, return_counts=True)
            
            total_count = counts.sum().float()
            alphas = (total_count / counts.float()) / len(unique_tokens)
            
            token_alphas = torch.ones_like(shift_labels_masked, dtype=torch.float32, device=device)
            for token, alpha in zip(unique_tokens, alphas):
                token_alphas[shift_labels_masked == token] = alpha.item()
            
            weights[is_special] = token_alphas[is_special] * focal_weight[is_special]
        else:
            weights[is_special] = focal_weight[is_special]
